...
3:      Dim StrnNav, Strn, Naklad, Odvod, Trzba, Navstev, PomocDat, OdvodE, OdvodDPH, AVF As Double
...
216:                    'Odvod = CDbl(DataGridView1.Rows(i2).Cells(3).Value) - CDbl(DataGridView1.Rows(i2).Cells(5).Value) - CDbl(DataGridView1.Rows(i2).Cells(7).Value)
                        AVF = Math.Round(DataGridView1.Rows(i).Cells(3).Value * frmSpracovavanie.PercentOdvoduDoFondov, 2) 'Math.Round(DataGridView1.Rows(i).Cells(4).Value * 0.03, 2)
                        DataGridView1.Rows(i).Cells(7).Value = Format(AVF, "##0.00")

217:                    Odvod = (CDbl(.Rows(i).Cells(3).Value) - CDbl(.Rows(i).Cells(5).Value) - AVF)
218:                    .Rows(i).Cells(8).Value = Format(Odvod, "##0.000")
219:
220:                    If ZobrazitFilmy(i2).PevnePoz <> 0 Then
221:                        Odvod = ZobrazitFilmy(i2).PevnePoz
222:                    Else
223:                        .Rows(i).Cells(9).Value = ZobrazitFilmy(i2).PercentPoz
...
228:                        .Rows(i).Cells(10).Value = Format(0, "##0.000")
229:                        ' .Rows(i).Cells(11).Value = Format(0, "###0.00")
230:                        '  .Rows(i).Cells(12).Value = Format(0, "###0.00")
231:                    Else
232:                        .Rows(i).Cells(10).Value = Format(Odvod, "##0.000")
233:                        '   .Rows(i).Cells(11).Value = Format(Odvod * frmSpracovavanie.pDPH / 100, "###0.00")
234:                        '   .Rows(i).Cells(12).Value = Format(Odvod + Odvod * (frmSpracovavanie.pDPH / 100), "###0.00")
235:                    End If
236:                    Exit For
237:                End If
238:            Next i2
...
257:            Naklad += CDbl(.Rows(i).Cells(8).Value)
258:            OdvodE += CDbl(.Rows(i).Cells(10).Value)

                AVF += CDbl(.Rows(i).Cells(7).Value)
261:        Next i
262:    End With

259:    OdvodDPH = Format(OdvodE * frmSpracovavanie.pDPH / 100, "###0.00") 'CDbl(.Rows(i).Cells(11).Value) 'Format(Odvod * frmSpracovavanie.pDPH / 100, "###0.00")
260:    Odvod = Format(OdvodE + OdvodE * (frmSpracovavanie.pDPH / 100), "###0.00") 'CDbl(.Rows(i).Cells(12).Value) 'Format(Odvod + Odvod * (frmSpracovavanie.pDPH / 100), "###0.00")
...
1:      If frmSpracovavanie.ChybaListky Or frmSpracovavanie.ChybaPredstavenia Or frmSpracovavanie.ChybaFilmy Then MsgBox("Poskodene data. Vystup nemusi byt spravny.", MsgBoxStyle.Exclamation) : Exit Sub
        frmMenu.Vytlacit = ""
2:      frmMenu.Velkost = 9 '8
3:      Dim ODDELOVAC As String = StrDup(90, "─") '105
4:      Dim ODDELOVAC2 As String = StrDup(16, " ") & StrDup(90 - 16, "─")
...
16:     Dim Odsadenie(15) As Integer
17:
18:
...
61:         Tab(6) = Tab(5) + 7 '51
            Tab(7) = Tab(6) + 8 '58
62:         Tab(8) = Tab(7) + 8 '66
63:         Tab(9) = Tab(8) + 10 '76
64:         Tab(10) = Tab(9) + 4 '80
65:         'Tab(11) = 88
66:         'Tab(12) = 97
...
75:         Hlavicka(8) = " Trzba"
76:         Hlavicka(9) = "%"
77:         Hlavicka(10) = "  Odvod"
78:         '  Hlavicka(11) = "  Odvod"
79:         '  Hlavicka(12) = "  Odvod"
80:
...
90:         'Hlavicka
91:         'frmMenu.Vytlacit += "---------1---------2---------3---------4---------5---------6---------7---------8---------9---------0-------" & Chr(13) & Chr(10)
            'Pomocka
            'PomocRetazec = ""
            'For i = 0 To 10 '12
            'PomocRetazec += StrDup((Tab(i) - Len(PomocRetazec)), " ") & i
            'Next i
            'frmMenu.Vytlacit += PomocRetazec & Chr(13) & Chr(10)
92:         'frmMenu.Vytlacit += "12345678901234567890123456789012345678901234567890123456789012345678901234567890123456789012345678901234567" & Chr(13) & Chr(10)
...
113:        Hlavicka(10) = "  (EUR)"
114:        '  Hlavicka(11) = "   DPH"
115:        ' Hlavicka(12) = "  spolu"
116:
117:        PomocRetazec = ""
118:        For i = 3 To 10 ' 12
119:            If (Not i = 4 And Not i = 6 And Not i = 9) Then
...
165:                        For i = 1 To 10 '12
166:                            PomocRetazec += StrDup((Tab(i) - Len(PomocRetazec)), " ")
167:
                                If i >= 3 And i <> 9 Then
                                    PomocRetazec += StrDup((Odsadenie(i) - Len(CStr(.Rows(X).Cells(i).Value))), " ")
                                End If
...
180:                        For i = 2 To 10 '12
181:                            If i <> 9 Then
182:                                PomocRetazec += StrDup((Tab(i) - Len(PomocRetazec)), " ")
183:
                                    If i = 3 Or i = 5 Or i = 7 Or i = 8 Or i >= 10 Then
                                        PomocRetazec += StrDup((Odsadenie(i) - Len(Format(sucty(i), "##0.00"))), " ")
                                    ElseIf i = 4 Or i = 6 Then
                                        PomocRetazec += StrDup((Odsadenie(i) - Len(Format(sucty(i), "##0"))), " ")
                                    End If

186:                                If i = 3 Or i = 5 Or i = 7 Or i >= 8 Then
...
209:        PomocRetazec += StrDup((Odsadenie(3) - Len(Format(CDbl(lblTrzba.Text), "##0.00"))), " ") & Format(CDbl(lblTrzba.Text), "##0.00")
210:        PomocRetazec += StrDup((Tab(4) - Len(PomocRetazec)), " ")
211:        PomocRetazec += StrDup((Odsadenie(4) - Len(Format(CDbl(lblNavstev.Text), "##0"))), " ") & Format(CDbl(lblNavstev.Text), "##0")
212:        PomocRetazec += StrDup((Tab(5) - Len(PomocRetazec)), " ")
213:        PomocRetazec += StrDup((Odsadenie(5) - Len(Format(CDbl(lblStrn.Text), "##0.00"))), " ") & Format(CDbl(lblStrn.Text), "##0.00")
214:        PomocRetazec += StrDup((Tab(6) - Len(PomocRetazec)), " ")
215:        PomocRetazec += StrDup((Odsadenie(6) - Len(Format(CDbl(lblStrnNav.Text), "##0"))), " ") & Format(CDbl(lblStrnNav.Text), "##0")
            PomocRetazec += StrDup((Tab(7) - Len(PomocRetazec)), " ")
            PomocRetazec += StrDup((Odsadenie(7) - Len(Format(CDbl(lblAVF.Text), "##0.00"))), " ") & Format(CDbl(lblAVF.Text), "##.00")
216:        PomocRetazec += StrDup((Tab(8) - Len(PomocRetazec)), " ")
217:        PomocRetazec += StrDup((Odsadenie(8) - Len(Format(CDbl(lblNakl.Text), "##0.00"))), " ") & Format(CDbl(lblNakl.Text), "##0.00")
218:        PomocRetazec += StrDup((Tab(10) - Len(PomocRetazec)), " ")
219:        PomocRetazec += StrDup((Odsadenie(10) - Len(Format(CDbl(lblOdvod.Text), "##0.00"))), " ") & Format(CDbl(lblOdvod.Text), "##0.00")
220:        ' PomocRetazec += StrDup((Tab(11) - Len(PomocRetazec)), " ")
221:        ' PomocRetazec += StrDup((Odsadenie(11) - Len(Format(CDbl(lblOdvodDPH.Text), "##0.00"))), " ") & Format(CDbl(lblOdvodDPH.Text), "##0.00")
222:        'PomocRetazec += StrDup((Tab(12) - Len(PomocRetazec)), " ")
223:        'PomocRetazec += StrDup((Odsadenie(12) - Len(Format(CDbl(lblOdvodSpolu.Text), "##0.00"))), " ") & Format(CDbl(lblOdvodSpolu.Text), "##0.00")
224:        frmMenu.Vytlacit += PomocRetazec & Chr(13) & Chr(10)

            PomocRetazec = "DPH:"
            PomocRetazec += StrDup((Tab(10) - Len(PomocRetazec)), " ")
            PomocRetazec += StrDup((Odsadenie(10) - Len(Format(CDbl(lblOdvodDPH.Text), "##0.00"))), " ") & Format(CDbl(lblOdvodDPH.Text), "##0.00")
            frmMenu.Vytlacit += PomocRetazec & Chr(13) & Chr(10)
            PomocRetazec = "Pozicovne spolu:"
            PomocRetazec += StrDup((Tab(10) - Len(PomocRetazec)), " ")
            PomocRetazec += StrDup((Odsadenie(10) - Len(Format(CDbl(lblOdvodSpolu.Text), "##0.00"))), " ") & Format(CDbl(lblOdvodSpolu.Text), "##0.00")
            frmMenu.Vytlacit += PomocRetazec & Chr(13) & Chr(10)
...
358:        PomocRetazec += StrDup((Odsadenie(3) - Len(Format(Vysledky(1), "##0.00"))), " ") & Format(Vysledky(1), "##0.00")
359:        PomocRetazec += StrDup((Tab(4) - Len(PomocRetazec)), " ")
360:        PomocRetazec += StrDup((Odsadenie(4) - Len(Format(Vysledky(2), "##0"))), " ") & Format(Vysledky(2), "##0")
361:        PomocRetazec += StrDup((Tab(5) - Len(PomocRetazec)), " ")
362:        PomocRetazec += StrDup((Odsadenie(5) - Len(Format(Vysledky(3), "##0.00"))), " ") & Format(Vysledky(3), "##0.00")
363:        PomocRetazec += StrDup((Tab(6) - Len(PomocRetazec)), " ")
364:        PomocRetazec += StrDup((Odsadenie(6) - Len(Format(Vysledky(4), "##0"))), " ") & Format(Vysledky(4), "##0")
365:        PomocRetazec += StrDup((Tab(7) - Len(PomocRetazec)), " ")
366:        PomocRetazec += StrDup((Odsadenie(7) - Len(Format(Vysledky(5), "##0.00"))), " ") & Format(Vysledky(5), "##0.00")
367:        PomocRetazec += StrDup((Tab(8) - Len(PomocRetazec)), " ")
368:        PomocRetazec += StrDup((Odsadenie(8) - Len(Format(Vysledky(9), "##0.00"))), " ") & Format(Vysledky(9), "##0.00")
369:        'PomocRetazec += StrDup((Tab(9) - Len(PomocRetazec)), " ")
370:        'PomocRetazec += StrDup((Odsadenie(9) - Len(Format(Vysledky(7), "##0.00"))), " ") & Format(Vysledky(7), "##0.00")
371:        PomocRetazec += StrDup((Tab(10) - Len(PomocRetazec)), " ")
372:        PomocRetazec += StrDup((Odsadenie(10) - Len(Format(Vysledky(8), "##0.00"))), " ") & Format(Vysledky(8), "##0.00")
